...
pub struct SysConfig {
    pub autogenerated: Option<(bool, String)>,
...
pub fn get_keymap() -> Option<String> {
    let content = fs::read_to_string("/etc/vconsole.conf").ok()?;
...
impl Default for SysConfig {
    fn default() -> Self {
        ...
        Self {
            autogenerated: Some((
                true,
                "This config was auto-generated. Make some changes ;)".to_string(),
            )),
            packages: Some(get_explicit_packages()),
            shells: Some(get_shells()),
            default_user: users.first().map(|user| user.user.clone()),
            users: Some(users),
...
impl SysConfig {
    ...
    pub fn read_or_clone_other_config(path: &str, cfg: &SysConfig) -> SysConfig {
        if !Path::new(path).exists() {
            return cfg.clone();
        }
